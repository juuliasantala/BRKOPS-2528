...
class PingTestcase(aetest.Testcase):
    ...
    def ping(self, steps, ping_job):
        ...
        with steps.start(
            f"Checking Ping from {ping_job['request']['serial']} to {ping_job['request']['target']}",
            continue_=True
                ) as step:
                if self.results["loss"]["percentage"] == 100: # If package loss is 100 %, fail
                    step.failed("100 % packet loss")
                else:
                    step.passed(f"Packet loss {self.results['loss']['percentage']} %")
...
if __name__ == "__main__":
    ...
    # their serial number
    serials = (os.getenv("MERAKI_SERIAL"),)
